...
SAMPLES_TTL = Path("D:\\0Programmes\\Ashrae\\si-builder\\samples\\ttl")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="base name of TTL files to load (data and schema)",
    )
    ...
    models = [
        *glob.glob(str(S223_FOLDER / "models" / "*.ttl")),
    ...
    for fname in args.name:
        datafile = SAMPLES_TTL / f"{fname}.data.ttl"
        schemafile = SAMPLES_TTL / f"{fname}.schema.ttl"
        logger.info(f"Loading files {datafile} and {schemafile}")

        if fname == "-":
            data_graph.parse(sys.stdin, format="turtle")
        else:
            logger.debug("loading %r", fname)
            data_graph.parse(datafile, format="turtle")
            data_graph.parse(schemafile, format="turtle")
