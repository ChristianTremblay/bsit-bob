...
zone_df=pd.read_csv('/Users/lazlopaul/Desktop/brick/LPbricktesting/b59_UFT_ahu_rm2.csv')
r_mat = {} #rtu matrix
...
for floor in range(3,5):
    ...
    for rtu in range(1,5):
        ...
        if floor == 3:
        	...
        	r_mat[rtu].supplyAirOutlet.mapsTo.connectsThrough >> plenum[floor][rtu].supplyInlet
        	r_mat[rtu].returnAirInlet.mapsTo.connectsThrough << ceiling_space[floor][rtu].returnOutlet
...
for i, row in zone_df.iterrows(): #iterating through UFTs
    ...
    for j,room_label in enumerate(rooms):
        ...
        room_supply_cp = AirInletConnectionPoint(HVACrooms[room_label], label=HVACrooms[room_label].label + ".supplyAir")
        room_return_cp = AirOutletConnectionPoint(HVACrooms[room_label], label=HVACrooms[room_label].label + ".returnAir")
        try:
