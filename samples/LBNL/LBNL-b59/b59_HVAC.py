...
from bob.systems.archives.coolingcoil import ChilledWaterCoil2, ChilledWaterCoil
...
from bob.sensor.temperature import WaterTemperatureSensor, AirTemperatureSensor, TemperatureSetpoint
...
class RooftopUnit(System):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        ra_pres = AirStaticPressureSensor(label=self.label+".RA_static_pressure", unit = qudt.PA)
        self > ra_pres
        ...
        self >pre_filter
        pf_press = AirStaticPressureSensor(label=self.label + 'PF_differential_pressure', unit = qudt.PA) # Do I need a different pressure sensor
        self >pf_press
        ...
        self >final_filter
        ff_press = AirStaticPressureSensor(label = self.label + '.FF_differential_pressure', unit = qudt.PA)
        self >ff_press
        ...
        sa_press = AirStaticPressureSensor(label = self.label + '.SA_static_pressure', unit = qudt.PA)
        self > sa_press
...
class UFT(System): #Underfloor Fan Terminal Unit
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        hw_coil = HotWaterCoilUFT(label=self.label+".hw_coil")           #problem using HotWaterCoil2, mapping a system connection point to a system connection point that maps to a device connection point
        supply_fan >> hw_coil
        self>hw_coil
        self.supplyAirOutlet.mapsTo = hw_coil.hot_water_coil.airOutlet #have to map system to the device connection point in the subsystem, does this make sense?    
...
class CoolingPump(Device):
    ...
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.airFlow.unit = unit.FT3_PER_MIN
class CoolingWaterSystem(System): #my devices using cool water will be connected to a junction at CWSupply
    node_type= None
...
class CoolingTower(Device): #should I try to specify type of cooling tower (counterflow I think)
    node_type= None
    waterInlet: ChilledWaterInletConnectionPoint
    waterOutlet: ChilledWaterOutletConnectionPoint
