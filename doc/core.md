...
- Node: Base for all model elements.
  ...
  - Operator shorthands:
    ...
    - a += x: add aspect/role
    - prop @ ref: add external reference to a Property
...
- Property(Node): Base property with specializations:
  ...
  - Setpoint
- ExternalReference(Node): Base for external references; attach with property @ ref.
- PhysicalSpace(Container, Node): s223:PhysicalSpace
- Zone(Container, Node), ZoneGroup(Container, Node), DomainSpace(Connectable)
- EnumerationKind(Node): Lightweight “class factory” for enumerations (Medium, Role, Domain, etc.)
